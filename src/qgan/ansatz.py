...
def get_ansatz_type_circuit(type_of_ansatz: str, size: int, layer: int) -> tuple[callable, int]:
    ...
    if type_of_ansatz == "XX_YY_ZZ_Z":
        count = count_gates_XX_YY_ZZ_Z(size, layer)
        return construct_qcircuit_XX_YY_ZZ_Z, count

    if type_of_ansatz == "ZZ_X_Z":
        count = count_gates_ZZ_X_Z(size, layer)
        return construct_qcircuit_ZZ_X_Z, count

    raise ValueError("Invalid type of ansatz specified.")


def count_gates_ZZ_X_Z(size: int, layer: int) -> int:
    """
...
def construct_qcircuit_ZZ_X_Z(size: int, layer: int, theta: list[int]):
...
def count_gates_XX_YY_ZZ_Z(size: int, layer: int) -> int:
    """
    Count gates in order to make uniform random angles for the gates (0 to 2*pi)
    """
...
def construct_qcircuit_XX_YY_ZZ_Z(size: int, layer: int, theta: list[int]):
    # If extra ancilla is used, different than ansatz, we reduce the size by 1,
    # to implement the ancilla logic separately.
    if CFG.extra_ancilla:
        size -= 1
